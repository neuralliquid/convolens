...
`BatonMcpClient` gains `listProjects(token)`, wrapping the `list_projects` MCP tool, using the same `this.batonResource` and the same client/timeout pattern already used by `searchTasks`/`createTask`. `this.batonResource` is not always-on: the constructor resolves it to `BATON_MCP_RESOURCE` (env override, else `https://mcp.baton.celladoresystems.com/mcp`) only when `BATON_OAUTH_MCP_ENABLED=true`, and to `''` otherwise — the empty string *is* the flag-off signal the rest of this document relies on. A requested `projectId` is accepted only if it appears in that call's result, called fresh — never from a cache — at each of two checkpoints: the picker-list route and `decide()`/project-changing `PATCH /:id`. `publish()` does not re-check (see Consequences — the D3 invariant makes a third *ConvoLens-side* call redundant, not a security requirement). That framing covers only the window between `decide()`/`update()` and `publish()` staying unchanged on ConvoLens's side; it does not by itself cover access being revoked in Baton during that window — see the fail-closed caveat in Consequences.

This live check only runs when `BATON_OAUTH_MCP_ENABLED=true` (i.e. `this.batonResource` is configured). With the flag off there is no Baton session to check against, so `decide()`/`update()` fall back to today's `normalizePinnedProjectId()` behavior unchanged: a synchronous equality check against the resolved default, no network call. This is not a weaker authorization path — with the flag off, `publish()` already refuses to reach Baton at all (`this.batonResource` empty → `TicketCandidateValidation`), so a pinned, unauthorized-but-inert `projectId` on a candidate that can never publish carries no risk. That rejection happens before `publish()` does anything with whatever token it was handed — there is no route- or service-level path where a supplied `x-baton-access-token` lets a flag-off publish reach Baton anyway; the `this.batonResource` check is unconditional and runs first. Accept-with-the-default-project and implicit publish both keep working exactly as they do today, with no token and no live check, precisely because a pinned-to-default `projectId` is all `publish()` ever needed. SPEC-002 §4.1 specifies the exact `decide()`/`update()` branch; §4.5 and §7 specify the corresponding accept and implicit-publish continuity.
...
- One additional Baton MCP round trip on `decide()`, project-changing `PATCH /:id`, and the new picker-list route, only when `BATON_OAUTH_MCP_ENABLED=true` — each bounded by the existing 15s timeout, not a new one. `publish()` adds none: the D3 invariant guarantees `projectId` cannot change between a successful `decide()`/`update()` and `publish()`, so re-checking membership on ConvoLens's side there would be a redundant round trip, not a security requirement.
- Skipping a publish-time recheck relies on an unverified assumption: that Baton's own `create_task` independently rejects a project the calling token can no longer access, if that access was revoked *after* `decide()`/`update()` and *before* `publish()` — a window this ADR does not otherwise close. Before Phase 1 ships with `BATON_OAUTH_MCP_ENABLED=true` in production, either confirm that `create_task` behavior against a live Baton deployment (an e2e test is enough — no code change needed if it holds), or add a publish-time membership check. Until confirmed, the "fail closed on revoked access" claim in Decision §2/§3 holds only for the picker-list/decide()/update() checkpoints, not for `publish()` itself.
...
Before enabling `BATON_OAUTH_MCP_ENABLED=true` in production: verify (via an e2e test against a real Baton deployment, or a written confirmation of `create_task`'s contract) that Baton's `create_task` itself rejects a project the caller's token no longer has access to — see the Consequences caveat on the `publish()` recheck gap above.
