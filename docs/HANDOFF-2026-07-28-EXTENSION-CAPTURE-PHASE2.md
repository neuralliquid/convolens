...
- Compatibility normalizes legacy captionless placeholders and scopes image-to-video correction to captures whose provenance proves they came from a pre-1.0.13 connector. The corrected message, compatibility hash, and content hash change atomically, while current image and video evidence remains distinct even when all other tuples match. The service also compares all overlapping stable participant identifiers, enriches name-only references without duplicating them, and preserves reconciliation warnings on exact repeats or later loss of stable scope.
...
- Packaged ZIP SHA-256: `3DAB1A0D3F5DAC1E64258BEE7E0617A7F2C366D7CE7C8CE1EC6143DAA7B51B9B`.
