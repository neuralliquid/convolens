...
- Content-script navigation compares the selected chat's verified JID or scoped header fallback and cancels pre-upload collection/review when the chat changes. Late collection responses are accepted only while the exact background operation remains in `collecting`, and stale content-script cleanup is scoped to its own operation ID so it cannot clear a newer chat's capture. If navigation occurs during an in-flight request, the background revalidates the tab's exact chat context before exposing `retry-required`; a mismatch cancels and discards the old payload. Page unload, tab closure, missing receivers, and background restart become explicit cancellation outcomes; restart restoration sends the cancelled snapshot to the affected tab before discarding its payload so page controls cannot remain stuck in an active state.
...
- Chrome extension Node tests: 50 passed, 0 failed, including shared-state, persistence-boundary, double-activation, navigation, retry-context and stale-collection cancellation, restart notification, teardown, serialized auth-owner replacement, exact-credential auth-owner binding, auth-transition invalidation, in-flight logout truth, terminal-render deduplication, page retry, and popup-reopen source contracts.
...
- Packaged ZIP SHA-256: `6FF33D29C819B963757A1DF4E12700A997394D04DBF33AA3D94B46558B848374`.
