...
- `decision === 'accepted' && BATON_OAUTH_MCP_ENABLED === 'true'` → use `getConvolensPublishTokens()`, forward `X-Baton-Access-Token`. (This is where a genuinely missing session surfaces as `401` from the existing helper, unchanged. `getConvolensPublishTokens()`'s own flag-off `503` case cannot trigger from this call site — the condition already requires `BATON_OAUTH_MCP_ENABLED === 'true'` before the helper is even called.)
